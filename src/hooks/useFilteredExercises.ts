...
export function useFilteredExercises() {
  ...
  return useMemo(() => {
    return exercises.filter((ex) => {
      ...
      if (filters.equipment !== 'all' && !ex.equipment.includes(filters.equipment)) return false;
      if (filters.location !== 'all' && !ex.location.includes(filters.location)) return false;
      if (filters.bodyPart !== 'all' && !ex.bodyPart.includes(filters.bodyPart)) return false;
      if (filters.search) {
